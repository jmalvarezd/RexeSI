...
public class MultiTeseoEaterLabyrinth extends MultiAgentLabyrinth {
    ...
    public MultiTeseoEaterLabyrinth(Vector<Agent> _agents, int[][] _structure) {
        ...
        paso = new int[_structure.length][_structure.length];
        for (int i = 0; i < _structure.length; i++) {
            for (int j = 0; j < _structure.length; j++) {
                paso[i][j] = 0;
            }
        }
    ...
    protected int agentIndex(Agent agent) {
        int i = 0;
        while (i < agents.size() && agents.get(i) != agent) {
            i++;
        }
        return i;
    }
            ...
            switch (actionID) {
                ...
                default:
                    if (actionID == 2) {
                        ...
                        paso[y][x]++;
                        pasos++;
                        //System.out.println("MATRIZ: " + pasos);
                        for (int j = 0; j < paso.length; j++) {
                            for (int k = 0; k < paso.length; k++) {
                          //      System.out.print(paso[j][k] + " ");
                            }
                           // System.out.println();
                        }
                    }
                    if (actionID != 2 || !((Boolean) p.getAttribute("afront")).booleanValue()) {
